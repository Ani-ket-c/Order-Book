...
void Book::setAnalyticsEngine(analytics::MicrostructureEngine* engine)
{
    analytics_ = engine;
}

void Book::updateAnalyticsOnTrade(bool aggressorBuy, int execPrice, int execQty)
{
    if (analytics_ == nullptr) {
        return;
    }

    const int bidPrice = highestBuy ? highestBuy->getLimitPrice() : 0;
    const int askPrice = lowestSell ? lowestSell->getLimitPrice() : 0;
    const int bidQty = highestBuy ? highestBuy->getTotalVolume() : 0;
    const int askQty = lowestSell ? lowestSell->getTotalVolume() : 0;

    analytics_->onTrade(aggressorBuy, execPrice, execQty, bidPrice, askPrice, bidQty, askQty);
}

void Book::updateAnalyticsFromBook()
{
    if (analytics_ == nullptr) {
        return;
    }
    ...
    analytics_->onCancel(bidPrice, askPrice, bidQty, askQty);
}
...
void Book::addLimitOrder(int orderId, bool buyOrSell, int shares, int limitPrice)
{
    ...
    if (shares != 0) [[likely]]
    {
        Order* newOrder = orderPool_.alloc(orderId, buyOrSell, shares, limitPrice);
...
void Book::addStopOrder(int orderId, bool buyOrSell, int shares, int stopPrice)
{
    ...
    if (shares != 0)
    {
        Order* newOrder = orderPool_.alloc(orderId, buyOrSell, shares, 0);
        orderMap.emplace(orderId, newOrder);
...
void Book::addLimit(int limitPrice, bool buyOrSell)
{
    ...
    Limit* newLimit = limitPool_.alloc(limitPrice, buyOrSell);
...
void Book::addStop(int stopPrice, bool buyOrSell)
{
    ...
    Limit* newStop = limitPool_.alloc(stopPrice, buyOrSell);
...
int Book::limitOrderAsMarketOrder(int orderId, bool buyOrSell, int shares, int limitPrice)
{
    if (buyOrSell) [[likely]]
    {
        Limit*& bookEdge = lowestSell;
        while (bookEdge != nullptr && shares != 0 && bookEdge->getLimitPrice() <= limitPrice)
        {
            Limit* currentLimit = bookEdge;
            int limitVolume = currentLimit->getTotalVolume();
            
            if (shares <= limitVolume)
            {
                // Execute remaining shares using inline marketOrderHelper logic
                while (bookEdge != nullptr && bookEdge->getHeadOrder()->getShares() <= shares)
                {
                    Order* headOrder = bookEdge->getHeadOrder();
                    const int execPrice = bookEdge->getLimitPrice();
                    const int execQty = headOrder->getShares();
                    ...
                    updateAnalyticsOnTrade(buyOrSell, execPrice, execQty);
                }
                if (bookEdge != nullptr && shares != 0)
                {
                    ...
                    executedOrdersCount += 1;
                    updateAnalyticsOnTrade(buyOrSell, execPrice, shares);
                }
                return 0;
            } else {
                // Execute entire limit level - use marketOrderHelper to consume all orders
                shares -= limitVolume;
                marketOrderHelper(orderId, buyOrSell, limitVolume);
            }
        }
        return shares;
    } else [[unlikely]]
    {
        Limit*& bookEdge = highestBuy;
        while (bookEdge != nullptr && shares != 0 && bookEdge->getLimitPrice() >= limitPrice)
        {
            Limit* currentLimit = bookEdge;
            int limitVolume = currentLimit->getTotalVolume();
...
{
    if (buyOrSell && lowestSell != nullptr && stopPrice <= lowestSell->getLimitPrice()) [[likely]]
    {
        executedOrdersCount = 0;
        AVLTreeBalanceCount = 0;
        ...
        return 0;
    } else if (!buyOrSell && highestBuy != nullptr && stopPrice >= highestBuy->getLimitPrice()) [[unlikely]]
    {
        executedOrdersCount = 0;
        AVLTreeBalanceCount = 0;
...
{
    Limit*& bookEdge = buyOrSell ? lowestSell : highestBuy;
...
void Book::updateLimitHeight(Limit* limit) {
